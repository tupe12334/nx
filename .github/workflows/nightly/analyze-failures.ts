...
export async function collectFailureDetails(
  ...
  for (const r of combined) {
    if (r.is_golden && r.status === 'failure') {
  ...
  for (const [project, val] of validations) {
    ...
    for (const tf of unreliable) {
      ...
      if (oldestJobId && oldestJobId !== 'null') {
        ...
        oldestHas = log.includes('FAIL') && log.includes(tf);
      }
      ...
      while (high - low > 1) {
        ...
        if (midJobId && midJobId !== 'null') {
          ...
          midHas = log.includes('FAIL') && log.includes(tf);
        }
        if (midHas) low = mid;
        else high = mid;
      }
  ...
  for (const project of sorted) {
    ...
    if (testFiles.length > 0) {
      for (const tf of testFiles) {
        ...
        lines.push(
          `📋 \`${tf}\` — failing since ${errorDate} (${errorDays} days)${label}`
        );
